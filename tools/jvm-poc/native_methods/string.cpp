...
NativeCallResult nm_string_indexOf_II(
    NativeCallContext& ctx, uint32_t /*pc*/, const std::vector<Value>& args) {
    static const Value kMissingReceiver = Value::named("<missing-receiver>");
    const Value& receiver = args.empty() ? kMissingReceiver : args[0];
    std::optional<uint32_t> id = objectId(receiver);
...
NativeCallResult caseConvert(NativeCallContext& ctx, const std::vector<Value>& args, bool lower) {
    static const Value kMissingReceiver = Value::named("<missing-receiver>");
    const Value& receiver = args.empty() ? kMissingReceiver : args[0];
    std::optional<uint32_t> id = objectId(receiver);
    auto strIt = id.has_value() ? ctx.strings.find(*id) : ctx.strings.end();
...
NativeCallResult nm_string_toLowerCase(
    NativeCallContext& ctx, uint32_t /*pc*/, const std::vector<Value>& args) {
    return caseConvert(ctx, args, true);
}

NativeCallResult nm_string_toUpperCase(
    NativeCallContext& ctx, uint32_t /*pc*/, const std::vector<Value>& args) {
    return caseConvert(ctx, args, false);
}
...
// All String methods are instance (init is invokespecial which is still
// dispatched through the instance leaf table in our model).
constexpr NativeMethodEntry kStringInstanceMethods[] = {
    {"init",        "([CII)V",                       &nm_string_init},
    {"length",      "()I",                           &nm_string_length},
    {"charAt",      "(I)C",                          &nm_string_charAt},
    {"indexOf",     "(II)I",                         &nm_string_indexOf_II},
...
NativeLeafFn lookupStringInstanceLeaf(const std::string& name, const std::string& descriptor) {
    return lookupLeaf(kStringInstanceMethods,
                      sizeof(kStringInstanceMethods) / sizeof(kStringInstanceMethods[0]),
                      name, descriptor);
}
...
NativeCallResult handleString(
    ...
    if (NativeLeafFn leaf = lookupStringInstanceLeaf(ref.name, ref.descriptor)) {
        ctx.callerLabel = methodLabel;
        return leaf(ctx, pc, args);
    }

    // Unknown String method: record an unsupported-string-call trace and
    // return a placeholder. Preserved as the catchall because the leaf table
    // only registers known methods — the per-class call-site cache will tag
    // unknown sites as kSlowCascade so this path still runs.
    static const Value kMissingReceiver = Value::named("<missing-receiver>");
    const Value& receiver = args.empty() ? kMissingReceiver : args[0];
