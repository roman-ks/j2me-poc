...
// Per-method leaf function pointer (resolved by resolveStaticLeaf /
// resolveInstanceLeaf). On dispatch-refactor branch this lived in
// class_file.hpp because the IC fast path read it directly; on this branch
// the typedef stays here (one of the lifted infrastructure pieces — not
// wired into dispatch yet).
using NativeLeafFn = NativeCallResult(*)(
    NativeCallContext& ctx,
    uint32_t pc,
    const std::vector<Value>& args);

// Per-class thick handler. Cached by ResolvedCallEntry; called from the
// interpreter's dispatch fast path. Signature takes MethodRefView (implicitly
// constructible from MethodRef so existing call sites keep working).
using NativeHandler = NativeCallResult(*)(
    NativeCallContext&, const std::string&, uint32_t,
    const MethodRefView&, const std::vector<Value>&);

// Returns the per-class thick handler for invokestatic on `className`, or
// nullptr if no className-only mapping exists (slow cascade in
// handleNativeStaticCall handles it).
NativeHandler resolveNativeStaticHandler(const std::string& className);

// Returns the per-class thick handler for invokevirtual on `className`.
// Receiver-type fallbacks (Image, String, Canvas subclass) are NOT cached —
// the slow cascade in handleNativeInstanceCall covers them.
